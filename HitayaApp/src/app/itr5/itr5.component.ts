...
@Component({
  selector: 'app-itr5',
  templateUrl: './itr5.component.html',
  styleUrls: ['./itr5.component.scss']
})
export class Itr5Component implements OnInit {
  ...
  submititr5(form: NgForm) {

    console.log("ITR5 Form");
    ...
    this.tax.UpdateTaxInformtion(this.tax_values).subscribe(
      responseLoginStatus => {
        ...
        if (this.status == 1) {
          this.router.navigate(['/tax-benefit-form']);
        }
        else {
      ...
      () => console.log("ITR5 Form method executed successfully")
    );
  }
