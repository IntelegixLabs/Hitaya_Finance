...
CREATE FUNCTION [dbo].[ufn_GetAllUser]()
RETURNS TABLE 
AS
	RETURN (SELECT * FROM USERDETAILS)
GO

IF OBJECT_ID('dbo.ufn_GetAllUser')  IS NOT NULL
DROP FUNCTION dbo.ufn_GetAllUser

SELECT * FROM dbo.ufn_GetAllUser()


CREATE FUNCTION [dbo].[ufn_GetSpecificTax](@PAN_CARD VARCHAR(20), @FILLING_YEAR VARCHAR(20))
RETURNS TABLE 
AS
	RETURN (SELECT TOP 1 * FROM TAX_FILLING WHERE PAN_CARD=@PAN_CARD AND FILLING_YEAR=@FILLING_YEAR)
GO

IF OBJECT_ID('dbo.ufn_GetSpecificTax')  IS NOT NULL
DROP FUNCTION dbo.ufn_GetSpecificTax
GO


SELECT * FROM dbo.ufn_GetSpecificTax('CGDPD1480N', '2021-2023')
